...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM tickets WHERE user_id = '".$_SESSION['login']."' AND msg LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM tickets WHERE user_id = '".$_SESSION['login']."' AND msg LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM tickets WHERE user_id = '".$_SESSION['login']."' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";	
	} else {
		$query_list = "SELECT * FROM tickets WHERE user_id = '".$_SESSION['login']."' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM tickets WHERE user_id = '".$_SESSION['login']."' ORDER BY id DESC";
}
