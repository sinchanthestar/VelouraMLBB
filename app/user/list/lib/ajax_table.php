...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM users WHERE uplink LIKE '%".$_SESSION['login']."%' AND level LIKE 'Client' AND username LIKE '%".protect_input($_GET['search'])."%' OR full_name LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM users WHERE uplink LIKE '%".$_SESSION['login']."%' AND level LIKE 'Client' AND username LIKE '%".protect_input($_GET['search'])."%' OR full_name LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM users WHERE uplink LIKE '%".$_SESSION['login']."%' AND level LIKE 'Client' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";	
	} else {
		$query_list = "SELECT * FROM users WHERE level LIKE 'Client' AND uplink LIKE '%".$_SESSION['login']."%' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM users WHERE level LIKE 'Client' AND uplink LIKE '%".$_SESSION['login']."%' ORDER BY id DESC";
}
...
if (isset($_POST['add'])) {
  ...
  $data_target = $model->db_query($db, "*", "users", "id = '".$_POST['id']."'");
  if ($data_target['count'] == 0) {
    $_SESSION['result'] = array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Data tidak ditemukan.');
  } else {
    if ($model->db_update($db, "users", array('status' => $_POST['status']), "id = '".$_POST['id']."'") == true) {
