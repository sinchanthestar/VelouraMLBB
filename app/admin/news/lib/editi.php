...
if (!isset($_GET['id'])) {
	...
	$data_target = $model->db_query($db, "*", "invite", "id = '".mysqli_real_escape_string($db, $_GET['id'])."'");
...
<form class="form-horizontal" method="POST" id="form-add">
	...
	<div class="form-group">
		<label>Konten</label>
		<textarea class="form-control" name="saldo" rows="5"><?php echo $data_target['rows']['saldo'] ?></textarea>
	</div>
