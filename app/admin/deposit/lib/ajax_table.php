...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM deposits WHERE method_name LIKE '%".protect_input($_GET['search'])."%' OR phone LIKE '%".protect_input($_GET['search'])."%' OR post_amount LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM deposits WHERE method_name LIKE '%".protect_input($_GET['search'])."%' OR phone LIKE '%".protect_input($_GET['search'])."%' OR post_amount LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM deposits WHERE status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else {
		$query_list = "SELECT * FROM deposits ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM deposits ORDER BY id DESC";
}
