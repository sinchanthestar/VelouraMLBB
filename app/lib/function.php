...
function guard_input($input) { 
    global $db; 
    $result = mysqli_real_escape_string($db, trim(htmlspecialchars(htmlentities($input, ENT_QUOTES, 'UTF-8')))); return $result; 
} 

function save_input($input) { 
    global $db; 
    $result = mysqli_real_escape_string($db, preg_replace("/[^a-zA-Z0-9\ ]/", "", $input)); 
    return $result; 
} 
...
function str_rand($length = 15) { 
    $str = ""; 
    $characters = array_merge(range('A','Z'), range('a','z'), range('0','9')); $max = count($characters) - 1; 
    for ($i = 0; $i < $length; $i++) { 
        $rand = mt_rand(0, $max); 
        $str .= $characters[$rand]; 
    } 
...
function micro_time() { 
    class Timer { 
        ...
        public function getTime(): float { 
            ...
            $microseconds = $microsecondsEnd - $this->microsecondsStart; // now the integer section ($seconds) should be small enough // to allow a float with 6 decimal digits 
            return round(($seconds + $microseconds), 6); 
        } 
    } 
    $t = new Timer(); usleep(250); return 'Page Load '.$t->getTime().' second'; 
}
?>