...
class Model {
    public function db_query_new($name, $db, $select, $table, $where = null, $order = 'id DESC', $limit = '') {
		$name = mysqli_query($db, "SELECT ".$select." FROM ".$table." ORDER BY ".$order." ".$limit);
		if ($where <> null) {
			$name = mysqli_query($db, "SELECT ".$select." FROM ".$table." WHERE ".$where." ORDER BY ".$order." ".$limit);
		}
		if (mysqli_error($db)) {
			return false;
		} else {
			...
			$result = array('query' => $name, 'rows' => $rows, 'count' => mysqli_num_rows($query));
			return $result;
		}
	}
    ...
    public function db_query2($db, $select, $table, $where = null, $order = 'id DESC', $limit = '') {
		$quer = mysqli_query($db, "SELECT ".$select." FROM ".$table." ORDER BY ".$order." ".$limit);
		if ($where <> null) {
			$quer = mysqli_query($db, "SELECT ".$select." FROM ".$table." WHERE ".$where." ORDER BY ".$order." ".$limit);
		}
		if (mysqli_error($db)) {
			return false;
		} else {
			...
			$result = array('query' => $quer, 'rows' => $rows, 'count' => mysqli_num_rows($query));
			return $result;
		}
	}
