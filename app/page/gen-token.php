...
if (isset($_GET['search']) AND isset($_GET['category'])) {
	if (!empty($_GET['search']) AND !empty($_GET['category'])) {
		$query_list = "SELECT * FROM services WHERE service_name LIKE '%".protect_input($_GET['search'])."%' AND category_id LIKE '%".protect_input($_GET['category'])."%' AND status = '1' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM services WHERE service_name LIKE '%".protect_input($_GET['search'])."%' AND status = '1' ORDER BY id DESC";
	} else if (!empty($_GET['category'])) {
		$query_list = "SELECT * FROM services WHERE category_id LIKE '%".protect_input($_GET['category'])."%' AND status = '1' ORDER BY id DESC";
	} else {
		$query_list = "SELECT * FROM services WHERE status = '1' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM services WHERE status = '1' ORDER BY id DESC";
}
