...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND data LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND data LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' ORDER BY id DESC";
}
                        		...
                        		<h4 class="m-t-0 m-b-30 header-title"><i class="fa fa-history"></i> Riwayat Pemesanan</h4>
									...
									<div class="table-responsive">
                                        <table class="table table-bordered table-hover">
											...
											<tbody>
												<tr>
													<td><a href="javascript:;" onclick="modal_open('detail', '<?php echo $config['web']['base_url']; ?>order/detail/sosmed1.php?id=<?php echo $data_query['id'] ?>')" class="badge badge-info">#<?php echo $data_query['id']; ?></a></td>
