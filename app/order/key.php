...
if(isset($_POST['action'])){
    if($_POST['action'] == "reset"){
        if(isset($_POST['id'])){
            $cek = $model->db_query($db, "token", "id = '".$_POST['id']."'");
            if($model->db_delete($db, "sList", "access_key = '".$cek['rows']['access_key']."'") == true){
                $_SESSION['result_msg'] = array('alert' => 'info', 'title' => 'BERHASIL!', 'msg' => 'Token '.$_POST['access_key'].' Telah Direset', 'show' => true);
            }else{
...
if ($_GET) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM token WHERE uid = '".$_SESSION['login']."' AND access_key LIKE '%".protect_input($_GET['search'])."%' AND games LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM token WHERE uid = '".$_SESSION['login']."' AND access_key LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM token WHERE uid = '".$_SESSION['login']."' AND games LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else {
		$query_list = "SELECT * FROM token WHERE uid = '".$_SESSION['login']."' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM token WHERE uid = '".$_SESSION['login']."' ORDER BY id DESC";
}
									...
									<div class="table-responsive">
                                        <table class="table table-bordered table-hover">
											...
											if($data_query['serial'] == 0) {
											    ...
											    $serial = $data_query['device'];
											}
											if($data_query['active'] == 0){
											    $dur = 'BELUM BERJALAN';
											}else if(date("Y-m-d H:i:s") >= $data_query['cheat_exp']){
											    $dur = 'EXPIRED!!';
											}else{
                                              $dur = $sisa;
                                            }
