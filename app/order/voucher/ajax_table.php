...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM vouchers WHERE uplink LIKE '%".$_SESSION['login']."%' AND voucher_code LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM vouchers WHERE uplink LIKE '%".$_SESSION['login']."%' AND voucher_code LIKE '%".protect_input($_GET['search'])."%' ORDER BY id DESC";
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM vouchers WHERE uplink LIKE '%".$_SESSION['login']."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY id DESC";
	} else {
		$query_list = "SELECT * FROM vouchers WHERE uplink LIKE '%".$_SESSION['login']."%' ORDER BY id DESC";
	}
} else {
	$query_list = "SELECT * FROM vouchers WHERE uplink LIKE '%".$_SESSION['login']."%' ORDER BY id DESC";
}
	...
	if (check_input($_POST, $input_name) == false) {
		...
		if (check_empty($empty) == true) {
			...
			if ($model->db_query($db, "voucher_code", "vouchers", "voucher_code = '".$input_post['voucher_code']."'")['count'] > 0) {
			    ...
			    if($model->db_query($db, "*", "users", "id = '".$_SESSION['login']."'")['rows']['balance'] > $_POST['amount']){
			        if($model->db_update($db, "users", array('balance' => $model->db_query($db, "*", "users", "id = '".$_SESSION['login']."'")['rows']['balance'] - $_POST['amount']), "id = '".$_SESSION['login']."'")){
